<?php

//Const for general
const ID = "id";
const ROLE_ADMIN = "Admin";
const ROLE_MEMBER = "Member";
const STATUS_ALIVE = true;
const STATUS_PASSED_AWAY = false;
const PASSWORD = "password";


// Const for person personal data
const PERSON_FIRST_NAME = "firstName";
const PERSON_LAST_NAME = "lastName";
const PERSON_NIK = "nik";
const PERSON_BIRTH_DATE = "birthDate";
const PERSON_INTERNAL_NOTE = "internalNote";
const PERSON_EMAIL = "email";
const PERSON_LAST_LOGGED_IN = "lastLoggedIn";
const PERSON_SEX = "sex";
const PERSON_ROLE = "role";
const PERSON_STATUS = "status";

//const for identified sex
const SEX_FEMALE = "Female";
const SEX_MALE = "Male";
const SEX_BETTER_NOT_SAY = "Better not say";

//cons for identified categories
const CATEGORIES_CHILD = "Children";
const CATEGORIES_PRODUCTIVE_AGE = "Productive ages";
const CATEGORIES_ALL = "All";
const CATEGORIES_ELDERLY = "Elderly";
const CATEGORIES_PASSED_AWAY = "Passed away";
